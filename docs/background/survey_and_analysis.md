...
This survey examines the landscape of 3D scene representation systems designed for or applicable to robotic manipulation, with particular focus on object-level SLAM, dynamic scene reconstruction during manipulation, and semantic scene graphs. The analysis is centered around a specific system (hereafter "Dynamic Scene Graph") that combines VLM-based detection (OWL-ViT + SAM), per-object TSDF volumes with a background TSDF, manipulation-phase-aware tracking (ICP during grasping/placing, end-effector propagation during movement), camera pose refinement via ICP against accumulated object clouds, and a geometric semantic scene graph with on/in/under/contain relations propagated during manipulation.
...
- Co-Fusion detects motion via geometric residuals; Dynamic Scene Graph uses a state machine based on gripper width. Co-Fusion generalizes to arbitrary moving objects but is noisier; Dynamic Scene Graph's approach is manipulation-specific and more robust for that domain.
- Co-Fusion performs joint camera + object pose optimization via direct image alignment; Dynamic Scene Graph does separate ICP-based pose refinement. Co-Fusion's formulation is more principled.
...
**Comparison with Dynamic Scene Graph.**
- MaskFusion is the closest architectural ancestor to Dynamic Scene Graph's detection-then-track pipeline. Key difference: surfels (MaskFusion) vs. TSDF volumes (Dynamic Scene Graph).
- MaskFusion uses direct photometric + geometric tracking; Dynamic Scene Graph uses ICP during specific phases.
- Dynamic Scene Graph's use of OWL-ViT + SAM instead of Mask R-CNN gives open-vocabulary detection capability, which is a significant practical advantage.
...
**Comparison with Dynamic Scene Graph.**
- MidFusion's factor graph formulation is the key differentiator: it *jointly* optimizes camera and object poses. Dynamic Scene Graph first refines camera pose via ICP, then separately updates object poses.
...
**Comparison with Dynamic Scene Graph.**
- ConceptGraphs focuses on *semantic scene graph construction* from **static** scenes; it does not track dynamic manipulation. Dynamic Scene Graph's scene graph is manipulation-aware and propagates state changes.
- ConceptGraphs stores *point clouds* per object rather than TSDF volumes — no mesh extraction.
- ConceptGraphs uses *CLIP features* for rich language grounding; Dynamic Scene Graph uses OWL-ViT scores.
- ConceptGraphs' edge relations are determined by LLM reasoning; Dynamic Scene Graph computes geometric relations from 3D bounding box overlap.
- ConceptGraphs assumes known camera poses; Dynamic Scene Graph refines poses.
...
**Comparison with Dynamic Scene Graph.**
- BundleSDF focuses on a *single object*; Dynamic Scene Graph manages a *multi-object scene*. They are complementary.
- BundleSDF's pose graph + bundle adjustment is more sophisticated than Dynamic Scene Graph's frame-by-frame ICP.
...
**Comparison with Dynamic Scene Graph.**
- FoundationPose is a *pose estimation* system, not a scene representation system. Could serve as a drop-in replacement for Dynamic Scene Graph's ICP-based pose tracking.
- One could use Dynamic Scene Graph's reconstructed mesh as input to FoundationPose for subsequent tracking.
...
**Comparison with Dynamic Scene Graph.**
- vMAP achieves *joint optimization* of camera pose, object poses, and object shape through differentiable rendering. This is the tight coupling Dynamic Scene Graph lacks.
...
## 3. What Makes Dynamic Scene Graph Unique
...
Dynamic Scene Graph maintains a scene graph with four geometric relation types computed from 3D bounding box overlap. When a parent object is moved, its *child objects* are propagated via stored relative transforms. This is not found in ConceptGraphs, OVSG, or other scene graph systems which treat the graph as a static semantic structure.
...
Dynamic Scene Graph occupies a unique position: it is the only system that combines open-vocabulary object-level TSDF reconstruction with manipulation-phase-aware tracking and a dynamic geometric scene graph with state propagation. Its closest relatives are TSDF++ (multi-object TSDF, no manipulation awareness), MaskFusion/Co-Fusion (dynamic multi-object tracking, no scene graph), and ConceptGraphs (semantic scene graph, no dynamic tracking).
...
The field is moving toward neural implicit/explicit representations (NeRF, 3DGS) and foundation-model-driven perception. Dynamic Scene Graph's per-object architecture is well-positioned to incorporate these: TSDF can be swapped for neural SDF or 3DGS per-object, and the VLM detection pipeline is already foundation-model-based.
