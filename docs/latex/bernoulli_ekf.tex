...
reset when the segmenter loses track -- its use as a soft association
prior is detailed in \S\ref{subsec:sam2_id}. All four fields are
produced by the open-vocabulary detector + video-segmenter pipeline.
...
For every pair $(i,l)\!\in\!\mathcal I_{k-1}\!\times\!\{1,\dots,M_k\}$
compute the per-pair innovation quantities
\begin{equation}
   \nu_{il} \;=\; \Log\!\bigl(T^{(i)\,-1}_{k|k-1}\,\hat T_{k,l}\bigr),\qquad
   S_{il}   \;=\; P^{(i)}_{k|k-1} + \hat R_{k,l},\qquad
   d^2_{il} \;=\; \nu_{il}^\top S_{il}^{-1}\nu_{il},
   \label{eq:innov_pair}
\end{equation}
i.e.\ \eqref{eq:ekf_innov}--\eqref{eq:mahal} under the substitution
$(z_k,R_k)\!\leftarrow\!(\hat T_{k,l},\hat R_{k,l})$ and track-state
substitution $(T_{k|k-1},P_{k|k-1})\!\leftarrow\!(T^{(i)}_{k|k-1},
P^{(i)}_{k|k-1})$. Define the cost matrix
$C\!\in\!(\R\cup\{+\infty\})^{|\mathcal I_{k-1}|\times M_k}$
\begin{equation}
   C_{il} \;=\;
   \begin{cases}
       d^2_{il}, & \hat\ell_{k,l}=\ell^{(i)}\ \text{and}\ d^2_{il}\le G_{\text{out}},\\
       +\infty,  & \text{otherwise},
   \end{cases}\label{eq:cost}
\end{equation}
so that infeasible pairs (wrong label, or outside the outer gate) are
forbidden. Let $\pi:\mathcal I_{k-1}\to\{0,1,\dots,M_k\}$ denote a
candidate assignment, with $\pi(i)\!=\!0$ encoding ``track $i$
unassigned'' and a one-to-one constraint on positive values. Define the
linear-assignment problem
\begin{equation}
   \pi^{\!*} \;=\;
   \argmin_{\pi}\;
   \sum_{i\in\mathcal I_{k-1}\,:\,\pi(i)>0} C_{i,\pi(i)}
   \qquad\text{s.t.}\quad
   \pi(i)=\pi(j)>0 \Longrightarrow i=j.
   \label{eq:hung}
\end{equation}
\eqref{eq:hung} is solved with scipy's \texttt{linear\_sum\_assignment}
(Jonker--Volgenant); any assignment that would incur cost $+\infty$ is
rewritten as unassigned ($\pi^{\!*}(i)\!=\!0$). Measurements with no
positive-assignment track are available for birth (\S\ref{sec:birth}).
This is the MAP collapse of the PMBM mixture over associations to its
single highest-weight component.

Note on circularity: $d^2_{il}$ depends on $(\hat T_{k,l},\hat R_{k,l})$,
which is produced by ICP against $\mathcal G^{(i)}_{k-1}$ -- which in
turn assumes we already know $i$. The orchestrator breaks this loop by
running ICP once per track on a \emph{restricted} candidate set (masks
whose world-frame centroid is within a coarse gate of $T^{(i)}_{k|k-1}$);
\eqref{eq:cost} then refines the selection using the refined pose
measurement. Masks failing the coarse gate for all tracks go to birth
with a centroid-initialised $\hat T_{k,l}$.

\subsection{SAM2 tracklet ID as a continuity prior}\label{subsec:sam2_id}

When the video segmenter is SAM2, each mask carries a tracklet
identifier $\tau_{k,l}\!\in\!\mathbb N$ that SAM2 propagates across
frames for the same object. Empirically the ``same-ID'' event is highly
informative -- two consecutive masks sharing a tracklet ID almost
always correspond to the same physical object -- while the
``different-ID'' event is silent: SAM2 loses track occasionally at
unpredictable frames, so a changed ID does not imply a different
object. We capture this asymmetry with one scalar,
$q_s\!\in\!(0.5,1)$, the per-frame probability that SAM2 preserves an
object's ID, and we give each track $i$ one auxiliary bookkeeping
variable $\tau^{(i)}_{k-1}\!\in\!\mathbb N$: the tracklet ID with which
$i$ was last associated.

Modelling $\tau_{k,l}$ as conditionally independent of the pose
measurement given track identity, with
$p(\tau_{k,l}\!=\!\tau^{(i)}_{k-1}\,\big|\,\text{track }i)=q_s$ and
$p(\tau_{k,l}\!\ne\!\tau^{(i)}_{k-1}\,\big|\,\text{track }i)=1-q_s$,
the augmented negative log-likelihood of pair $(i,l)$ adds, to $d^2_{il}$,
\[
   -2\log p\!\bigl(\tau_{k,l}\,\big|\,\tau^{(i)}_{k-1}\bigr)
   \;=\;
   \underbrace{-2\log(1-q_s)}_{\text{same for all }(i,l)}\;-\;
   \alpha\,\mathbb 1\!\bigl[\tau_{k,l}=\tau^{(i)}_{k-1}\bigr],
   \qquad
   \alpha \;\triangleq\; 2\log\frac{q_s}{1-q_s} \;>\;0.
\]
The first term is the same constant on every $(i,l)$ pair and drops
from the $\arg\min$ in \eqref{eq:hung}; only the indicator survives.
The cost matrix \eqref{eq:cost} is therefore modified by a single
additive bonus:
\begin{equation}
   C_{il} \;=\;
   \begin{cases}
...
\paragraph{Bookkeeping.} After solving \eqref{eq:hung}, for every
matched pair $\pi^{\!*}(i)\!=\!l$ set
$\tau^{(i)}_k\!\leftarrow\!\tau_{k,l}$; for a missed track
($\pi^{\!*}(i)\!=\!0$) keep $\tau^{(i)}_k\!=\!\tau^{(i)}_{k-1}$ so a
transient SAM2 drop does not erase the memory (SAM2 may return the
same ID a few frames later, regaining the bonus); at birth set
$\tau^{(i_{\mathrm{new}})}_k\!=\!\tau_{k,l}$. A never-previously-seen
tracklet ID satisfies $\tau_{k,l}\!\ne\!\tau^{(i)}_{k-1}$ for every
$i$, so its indicator vanishes column-wise and the cost reduces to
$d^2_{il}$ with no special-case handling.

\paragraph{Scope of the modification.} The ID prior enters only
through \eqref{eq:cost_sam2}; \S\ref{sec:update}, \S\ref{sec:birth},
and \S\ref{sec:vis} consume the refined assignment $\pi^{\!*}$
unchanged. In particular, the existence updates
\eqref{eq:r_assoc}/\eqref{eq:r_miss} still use only the pose likelihood
\eqref{eq:ekf_lik}; we do not fold $p(\tau|\tau^{(i)}_{k-1})$ into
$\mathcal L_{il}$. This avoids double-counting -- $\tau$ and $z_{k,l}$
are not strictly independent, both being deterministic functions of
the mask $m_{k,l}$ -- and keeps the existence recursion calibrated to
pose evidence only, using $\tau$ exclusively as a tie-breaker for
assignment.

\paragraph{Calibration and degeneracy.} $q_s$ is the empirical
per-frame probability that SAM2 preserves an object's ID, measured
once on a held-out sequence. Values in $[0.9,0.95]$ yield
$\alpha\!\in\![4.4,5.9]$: in $\chi^2_6$ units (gates
$G_{\text{in}}\!=\!12.59$, $G_{\text{out}}\!=\!25$), this magnitude
breaks ties between two geometrically ambiguous same-class tracks
without overriding a clear geometric mismatch. Limits: $\alpha\!=\!0$
($q_s\!=\!0.5$) recovers \eqref{eq:cost} exactly; $\alpha\!\to\!\infty$
($q_s\!\to\!1$) hard-binds SAM2 IDs and would force a spurious new
track on every SAM2 re-ID, which is not recommended given the failure
mode motivating this subsection.
...
\begin{enumerate}[leftmargin=2em]
   ...
   \item \textbf{Measure.} For every mask $m_{k,l}$ back-project to a world-frame point cloud $\mathcal C_{k,l}$. For each candidate track $i$ passing a coarse gate, run colored ICP to obtain $(\hat T_{k,l}^{(i)},\hat R_{k,l}^{(i)})$; compute $d^2_{il}$ via \eqref{eq:innov_pair} and $C_{il}$ via \eqref{eq:cost_sam2}.
...
\begin{itemize}[leftmargin=2em]
   ...
   \item Cost matrix \eqref{eq:cost_sam2}, assignment \eqref{eq:hung},
...
disappear), $p_d\!=\!0.9$, $q_s\!=\!0.9$ (SAM2 preserves IDs across
most frames; calibrate on a held-out sequence), $\lambda_c$ estimated
from a few static frames as the average per-frame detection count
divided by image area, $\lambda_b\!=\!\lambda_c$,
...
domain-specific; setting $q_s\!=\!0.5$ ($\alpha\!=\!0$) switches off
the SAM2 ID prior entirely and recovers the pure-geometry cost
\eqref{eq:cost}.
...
\item \textbf{SAM2 tracklet-ID augmentation} \emph{(modelling choice)}.
      Standard PMBM consumes only pose-like measurements; we augment
      each detection with the segmenter's tracklet ID $\tau_{k,l}$
      and a conditional-independence model
      $p(\tau|\tau^{(i)}_{k-1}) = q_s^{\mathbb 1[\tau=\tau^{(i)}_{k-1}]}(1-q_s)^{\mathbb 1[\tau\ne\tau^{(i)}_{k-1}]}$
      (\S\ref{subsec:sam2_id}). The ID likelihood enters the association
      cost \eqref{eq:cost_sam2} as an additive $\alpha$-bonus on matching
      IDs and is intentionally \emph{not} folded into the existence
      update, since $\tau$ and $z_{k,l}$ are not strictly independent
      (both derive from $m_{k,l}$) and double-counting would
      mis-calibrate \eqref{eq:r_assoc}. Internally Bayesian under the
      model; $\alpha\!=\!0$ recovers \eqref{eq:cost}.
...
Modelling choices A.3 -- A.6 and A.9 specialise or enrich the standard
PMBM model to our domain and are reversible: restoring the PPP,
Murty K-best mixture, spatially-adaptive birth, per-class $p_s$, or
disabling the SAM2 tracklet-ID bonus ($\alpha\!=\!0$) would re-recover
