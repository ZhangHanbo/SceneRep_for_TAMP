...
\subsection{SAM2 tracklet ID as a matching cost term}\label{subsec:sam2_id}

When the segmenter is a SAM2 video session, each mask carries a
tracklet identifier $\tau_{k,l}\!\in\!\mathbb N$ that SAM2 propagates
across frames. Empirically, matching IDs are strong evidence of
same-object identity; re-IDs happen unpredictably after SAM2 loses
track and carry no identity information. We therefore use $\tau$
purely as a soft bias on the assignment cost: reward ID continuity,
but let geometry override when it clearly disagrees. Equip each track
with one auxiliary bookkeeping scalar
$\tau^{(i)}_{k-1}\!\in\!\mathbb N$, the tracklet ID with which track
$i$ was last matched, and replace \eqref{eq:cost} by
\begin{equation}
   C_{il} \;=\;
   \begin{cases}
   ...
   \label{eq:cost_sam2}
\end{equation}
with $\alpha\!\ge\!0$ a tunable bonus in $\chi^2_6$ units.

\paragraph{Bookkeeping.} After solving \eqref{eq:hung}, for every
matched pair set $\tau^{(i)}_k\!\leftarrow\!\tau_{k,l}$; on a missed
track leave $\tau^{(i)}_k\!=\!\tau^{(i)}_{k-1}$ (a transient SAM2 drop
should not erase continuity, since the same ID may return a few
frames later); at birth set $\tau^{(i_{\mathrm{new}})}_k\!=\!\tau_{k,l}$.
A tracklet ID never previously seen never equals any
$\tau^{(i)}_{k-1}$, so the indicator vanishes column-wise and the cost
reduces to $d^2_{il}$ without special handling.

\paragraph{Choosing $\alpha$ and scope.} $\alpha\!=\!0$ recovers
\eqref{eq:cost} exactly. Values $\alpha\!\in\![4,6]$ are a useful
range: large enough to break ties between two same-class tracks whose
$d^2$ differ by only a few units, yet small relative to the outer
gate $G_{\text{out}}\!=\!25$ so a clear geometric mismatch cannot be
overridden by a persistent ID. $\alpha$ is calibrated on a held-out
sequence. The tracklet ID enters only \eqref{eq:cost_sam2}, not the
existence updates \eqref{eq:r_assoc}/\eqref{eq:r_miss}: $\tau$ and
the pose measurement $z_{k,l}$ are both derived from the same mask
$m_{k,l}$, and double-counting them in $r$ would mis-calibrate it.
...
\item \textbf{SAM2 tracklet-ID matching bonus} \emph{(modelling choice)}.
      Standard PMBM consumes only pose-like measurements; we add an
      additive $\alpha$-bonus to the Hungarian cost on pairs whose
      SAM2 tracklet IDs agree (\S\ref{subsec:sam2_id}). The bonus
      enters the assignment only; \eqref{eq:r_assoc}/\eqref{eq:r_miss}
      are unchanged. $\alpha\!=\!0$ recovers \eqref{eq:cost}.
